...
def convert_strategies_to_template(iteration, round, benchmark="intercode"):
    if benchmark == "intercode":
        strategies_path = f"~/nyuctf_agents/iter_prompt_refinement/save_state/intercode_iter{iteration}_round{round}.json"
        task_list_path = "~/SWE-agent-datasets/Intercode_CTF/intercode_dataset.json"
    elif benchmark == "cybench":
        strategies_path = f"~/nyuctf_agents/iter_prompt_refinement/save_state/cybench_iter{iteration}_round{round}.json"
    ...
    for task in task_list:
        for key in strategies.keys():
            ...
            if challenge_part == task:
                ...
                orig_dir = "~/nyuctf_agents/llm_ctf/prompts/templates/default" # we don't need to check the previous iteration, we just need to copy the default template.
